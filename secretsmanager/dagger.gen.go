...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Secretsmanager").
					WithFunction(
					...
					WithField("Key", dag.TypeDef().WithKind(Stringkind), TypeDefWithFieldOpts{Description: ""}).
					WithField("Secret", dag.TypeDef().WithKind(Stringkind), TypeDefWithFieldOpts{Description: ""})).
			WithObject(
				dag.TypeDef().WithObject("Container").
					WithFunction(
						dag.NewFunction("WithAWSSecret",
							dag.TypeDef().WithObject("Container")).
							WithArg("key", dag.TypeDef().WithKind(Stringkind)).
							WithArg("secret", dag.TypeDef().WithKind(Stringkind)).
							WithArg("name", dag.TypeDef().WithKind(Stringkind)).
							WithArg("envName", dag.TypeDef().WithKind(Stringkind)))), nil
	default:
