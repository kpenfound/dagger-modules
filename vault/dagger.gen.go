...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Vault":
		...
		case "Auth":
			...
			return (*Vault).Auth(&parent, approleId, approleSecret, address), nil
		case "GetSecret":
			var err error
			var parent Vault
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Vault).GetSecret(&parent, ctx, secret, key)
		case "PutSecret":
			var err error
			var parent Vault
			err = json.Unmarshal(parentJSON, &parent)
